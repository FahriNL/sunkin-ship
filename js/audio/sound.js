...
class SoundFX {
  constructor() {
    ...
    // Background Sea Ambience & Dynamic Battle Music
  ...
  set muted(val) {
    ...
    if (this.seaAmbienceAudio) {
      this.seaAmbienceAudio.volume = this._muted ? 0 : 0.14;
    }
    if (this.battleMusicAudio) {
      this.battleMusicAudio.volume = this._muted ? 0 : this.battleMusicVolume;
    }
  }
  ...
  updateBattleMusic(inHeavyBattle, dt) {
    ...
    if (this.battleMusicAudio) {
      this.battleMusicAudio.volume = this._muted ? 0 : Math.max(0, Math.min(1, this.battleMusicVolume));
